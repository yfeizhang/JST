...
#         first_dist = []
    ...
    def compute_coupling(self, entreg=None, gpu=None, m=None, **kwargs):
        ...
        if 'method' in kwargs and kwargs['method'] == 'emd':
            print("emd")
            π = ot.partial.partial_wasserstein(a, b, C / C.max(), m=m, nb_dummies=kwargs['nb_dummies'])
        elif not gpu:
            π = ot.partial.entropic_partial_wasserstein(a, b, C / C.max(), reg=entreg, m=m)
        else:
...
class FeatureCost():
    ...
    def __init__(self, src_embedding=None, tgt_embedding=None, src_dim=None,
                 tgt_dim=None, p=2, device='cpu', method=None):
    ...
    def __call__(self, X1, X2):
        ...
        if self.method == 'cosine':
            print('cosine')
            c = torch.bmm(X1, X2.transpose(1, 2)) / (torch.norm(X1, dim=2)[:, :, None] * torch.norm(X2, dim=2))
            c = 1 - c
        else:
            if self.p == 1:
