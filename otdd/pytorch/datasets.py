...
def load_torchvision_data_shuffle(dataname, valid_size=0.1, splits=None, shuffle=True,
                    ...
                    resize=None, to3channels=False,
                    maxsize = None, maxsize_test=None, num_workers = 0, transform=None,
                    data=None, datadir=None, download=True, shuffle_per=0, corrupted_per=0, std_dev=None):
    """ Load torchvision datasets.
...
#  ╚═════╝░╚═╝░░╚═╝░╚═════╝░╚═╝░░░░░╚═╝░░░░░╚══════╝╚══════╝
    ...
    if corrupted_per != 0:
        total_shuffles = int(corrupted_per * len(fold_idxs['train']))
        shuffle_inds = np.random.choice(sorted(fold_idxs['train']), size=total_shuffles, replace=False)
        if dataname == 'CIFAR10' or dataname == 'CIFAR100':
            print("CIFAR TEN or CIFAR HUNDRED")
            for index in shuffle_inds:
                new_img = add_gaussian_noise(train.data[index], std_dev)
                train.data[index] = new_img
        elif dataname == 'MNIST' or dataname == 'FashionMNIST':
            print("MNIST or FashionMNIST")
            for index in shuffle_inds:
                new_img = add_gaussian_noise(train.data[index], std_dev)
                train.data[index] = new_img
...
#             train.targets[index] = cur_label
    ...
    if shuffle_per != 0 or corrupted_per != 0:
